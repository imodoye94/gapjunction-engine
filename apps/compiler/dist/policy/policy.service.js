...
let PolicyService = (() => {
    ...
    var PolicyService = class {
         ...
         */
        async lintChannel(channel, orgId, acknowledgedViolations = []) {
            this.logger.debug('Starting policy lint', {
                channelId: channel.channelId,
                orgId,
                acknowledgedCount: acknowledgedViolations.length
            });
            const orgPolicy = await this.getOrgPolicy(orgId);
            const violations = [];
            // Security policy checks
            violations.push(...this.checkSecurityPolicies(channel, orgPolicy));
            // Nexon capability checks
            violations.push(...this.checkNexonCapabilities(channel, orgPolicy));
            // Runtime target checks
            violations.push(...this.checkRuntimeTarget(channel, orgPolicy));
            // Best practice checks
            violations.push(...this.checkBestPractices(channel, orgPolicy));
            // Compliance checks
            violations.push(...this.checkCompliance(channel, orgPolicy));
            ...
            this.logger.debug('Policy lint completed', {
                channelId: channel.channelId,
                passed,
                summary
            });
         ...
         */
        checkSecurityPolicies(channel, orgPolicy) {
            const violations = [];
            const security = channel.security || {};
            // Internet HTTP access check
            if (security.allowInternetHttpOut) {
                const severity = this.getSeverityFromPolicy(orgPolicy.policies.allowInternetAccess);
                if (severity) {
                    violations.push({
                        ruleId: 'SEC001',
            ...
            if (security.allowHttpInPublic) {
                const severity = this.getSeverityFromPolicy(orgPolicy.policies.allowPublicEndpoints);
                if (severity) {
                    violations.push({
                        ruleId: 'SEC004',
         ...
         */
        checkNexonCapabilities(channel, orgPolicy) {
            const violations = [];
         ...
         */
        checkRuntimeTarget(channel, orgPolicy) {
            const violations = [];
         ...
         */
        checkBestPractices(channel, orgPolicy) {
            const violations = [];
            // Check channel size
         ...
         */
        checkCompliance(channel, orgPolicy) {
            const violations = [];
            // Check for potential PHI handling stages
            const phiRiskyNexons = ['http.request', 'file.write', 'database.insert', 'email.send'];
            const phiRiskyStages = channel.stages.filter(stage => phiRiskyNexons.some(risky => stage.nexonId.includes(risky)));
        ...
        async getOrgPolicy(orgId) {
            ...
            this.logger.debug('Using default org policy', { orgId });
            return this.defaultOrgPolicy;
        }
        /**
         * Convert policy setting to severity level
         */
        getSeverityFromPolicy(policy) {
            switch (policy) {
    ...
    return PolicyService = _classThis;
})();
